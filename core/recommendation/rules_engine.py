...
class RecommendationRulesEngine:
    ...
    async def evaluate_policies(self, session: AsyncSession, customer_id: str) -> CustomerRecommendations:
        ...
        """
        logger.info(f"Evaluating credit policies for customer {customer_id}")
        ...
        if not intel:
            logger.warning(f"No customer intelligence found for {customer_id} during credit policy evaluation.")
