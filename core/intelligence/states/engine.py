...
class StateEngine:
    ...
    def compute(
        ...
        df = features_df.select(
            ["customer_id", "date", "events_window", "sales_window", "sales_recent", "active_duration_days"]
        )
        ...
        df = df.with_columns(pl.col("risk_score").alias("stress_score"))
        ...
        df = df.with_columns(
            ...
            .when(
                (pl.col("trust_score") > policy.elite_trust_threshold)
                & (pl.col("stress_score") < policy.elite_stress_threshold)
            )
            .then(pl.lit("elite"))
            .when(
                (pl.col("trust_score") > policy.active_trust_threshold)
                & (pl.col("stress_score") < policy.active_stress_threshold)
            )
            .then(pl.lit("active"))
        ...
        df = df.with_columns(
            ...
            .when(
                (pl.col("velocity_ratio") > policy.growing_ratio)
                & (pl.col("velocity_ratio") <= policy.accelerating_ratio)
            )
            .then(pl.lit("GROWING"))
            .when((pl.col("velocity_ratio") < policy.collapsing_ratio) & (pl.col("historical_velocity") > 0))
            .then(pl.lit("COLLAPSING"))
            .when(
                (pl.col("velocity_ratio") < policy.declining_ratio)
                & (pl.col("velocity_ratio") >= policy.collapsing_ratio)
            )
            .then(pl.lit("DECLINING"))
        ...
        df = df.with_columns(
            pl.when(
                ...
                & (
                    (pl.col("trajectory").is_in(["COLLAPSING", "DECLINING"]))
                    | (pl.col("stress_score") > policy.declining_stress_threshold)
                )
            )
            .then(pl.lit("declining"))
